...
client.on("ready", () => {
  ...
  let channel = client.channels.get('376352751912747013');
  	let tchannel = client.channels.get('376352751912747010');
...
client.on("message", (message) => {
  ...
  // [rest of the code]
		...
		if (message.content.startsWith(prefix + "fileplay") | message.author.id == '376352751912747010') {
  		...
  		// Monika SendMsg.
		if (message.content.startsWith(prefix + "sendmsg") | message.author.id == '376352751912747010') {
			if (!message.content.startsWith(prefix))
				return;
			var args = message.content.substring(prefix.length).split(" ");
			var msgtosend;
...
client.on('messageUpdate', function(omsg, nmsg){
	console.log(omsg.id);
  if(omsg.author.username == 'Mee6')
  {
	    	let tchannel = client.channels.get('376352751912747010');
						var rantamp;
			rantamp = Math.floor(Math.random() * 9) + 1;
			switch (rantamp) {
			case 1:
				tchannel.sendMessage("幹你娘");
				break;
			case 2:
				tchannel.sendMessage("耖你媽");
				break;
			case 3:
				tchannel.sendMessage("淦信不信我明天去美國共吼你死");
				break;
			case 4:
				tchannel.sendMessage("給我小心點");
				break;
			case 5:
				tchannel.sendMessage("殺小");
				break;
			case 6:
				tchannel.sendMessage("淦我明天直接去你家門");
				break;
			case 7:				
	            tchannel.sendMessage('淦林娘死Mee6 我還不打死你');
				break;
			case 8:
			    tchannel.sendMessage('不會講話的死機器人');
				playaudio('F:/Creative Things/Voices/Jane/簡之笑.mp3');
				break;
			case 9:
				tchannel.sendMessage('${159985870458322944} 你殺小');
				break;
			}
  }
});
...
{
	guild.defaultChannel.send('Hello! + user + !');
});
...
function playaudio(dic)
{
	 ...
	 channel.join()
     .then(connection => {
  return connection.playFile(dic);
 })
     .then(dispatcher => {
